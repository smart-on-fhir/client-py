...
class TestFHIRClientPagination(unittest.TestCase):
    ...
    @patch("requests.get")
    def test_execute_pagination_request_returns_none_on_no_next_link(self, mock_get):
        # Mock the response to simulate no next link
        mock_response = MagicMock()
        ...
        # Check that the result is None (since there is no next link)
