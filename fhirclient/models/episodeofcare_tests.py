...
class EpisodeOfCareTests(unittest.TestCase):
    def instantiate_from(self, filename):
        ...
        with io.open(os.path.join(datadir, filename), 'r', encoding='utf-8') as handle:
            js = json.load(handle)
        instance = episodeofcare.EpisodeOfCare(js)
        self.assertIsNotNone(instance, "Must have instantiated a test instance")
        return instance
    
    def testEpisodeOfCare1(self):
        inst = self.instantiate_from("episodeofcare-example.json")
        self.assertIsNotNone(inst, "Must have instantiated a <fhirclass.FHIRClass object at 0x10e2f2d10> instance")
    
        self.assertEqual(inst.careTeam[0].period.end.date, FHIRDate("2014-09-16").date)
        self.assertEqual(inst.careTeam[0].period.end.isostring, "2014-09-16")
        self.assertEqual(inst.careTeam[0].period.start.date, FHIRDate("2014-09-01").date)
        self.assertEqual(inst.careTeam[0].period.start.isostring, "2014-09-01")
        ...
        self.assertEqual(inst.period.start.isostring, "2014-09-01")
        self.assertEqual(inst.status, "active")
        self.assertEqual(inst.statusHistory[0].period.end.date, FHIRDate("2014-09-14").date)
        self.assertEqual(inst.statusHistory[0].period.end.isostring, "2014-09-14")
        self.assertEqual(inst.statusHistory[0].period.start.date, FHIRDate("2014-09-01").date)
        self.assertEqual(inst.statusHistory[0].period.start.isostring, "2014-09-01")
        ...
        self.assertEqual(inst.statusHistory[1].period.end.isostring, "2014-09-21")
        self.assertEqual(inst.statusHistory[1].period.start.date, FHIRDate("2014-09-15").date)
        self.assertEqual(inst.statusHistory[1].period.start.isostring, "2014-09-15")
        ...
        self.assertEqual(inst.statusHistory[2].period.end.isostring, "2014-09-24")
        self.assertEqual(inst.statusHistory[2].period.start.date, FHIRDate("2014-09-22").date)
        self.assertEqual(inst.statusHistory[2].period.start.isostring, "2014-09-22")
        ...
        self.assertEqual(inst.statusHistory[3].period.start.isostring, "2014-09-25")
